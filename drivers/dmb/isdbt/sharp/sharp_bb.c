...
#define FEATURE_ANT_LNA_USE_SNR_RSSI
...
// skip duplicated SetFequency
  if(freq_idx == set_frequency)
  {
    ISDBT_MSG_SHARP_BB("Same Freq.  Now[%d]  Prev[%d] Return !", freq_idx, set_frequency);
    scan.found = 1;
    return scan.found;
  }

  set_frequency = freq_idx;

// always ON LNA at Start channel
#ifdef FEATURE_DMB_LNA_CTRL
  if(!lna_ctrl)
  {
...
SIDE EFFECTS
======================================================================*/
static void sharp_bb_lna_ctrl(int sqindicator, int rssi)
{
  int lna_control;
#ifdef FEATURE_ANT_LNA_USE_SNR_RSSI
  int lna_arry[] = {-85, -68}; //{ON, OFF}
  lna_control = rssi;
#else
  int lna_arry[] = {35, 90}; //{ON, OFF}
  lna_control = sqindicator;
#endif

  if(lna_ctrl)
  {
    if(lna_control >= lna_arry[lna_ctrl])
    {
      //ISDBT_MSG_SHARP_BB("[%s] DMB_LNA OFF, lna_control[%d]\n", __func__, lna_control);
  ...
  else
  {
    if(lna_control <= lna_arry[lna_ctrl])
    {
      //ISDBT_MSG_SHARP_BB("[%s] DMB_LNA ON, lna_control[%d]\n", __func__, lna_control);
      dmb_set_gpio(DMB_LNA, 1);
      lna_ctrl = 1;
    }
  }
...
SIDE EFFECTS
======================================================================*/
int sharp_bb_get_ant_value(int sqindicator, int snr)
{
  static int saved_level = 0;
  int level = 0;
  int ant_control;
#ifdef FEATURE_ANT_LNA_USE_SNR_RSSI
  int ant_table[] = {5, 9, 13};
  int hystery_val[] = {1, 2, 2};
  ant_control = snr;
#else
  int ant_table[] = {7, 15, 25};
  int hystery_val[] = {3, 5, 5};
  ant_control = sqindicator;
#endif
  ...
  if(ant_control < ant_table[0]) level = 0;
  else if((ant_control >= ant_table[0] && ant_control < ant_table[1])) level = 1;
  else if((ant_control >= ant_table[1] && ant_control < ant_table[2])) level = 2;
  else if(ant_control >= ant_table[2]) level = 3;

  if(level == saved_level+1)
  {
    if(ant_control >= (ant_table[level-1]+hystery_val[level-1])) // ANT bigger case
    {
      saved_level = level;
    }
  }
...
#ifdef FEATURE_USE_AVG_BER
  sig_info->ber = status.dber;
#else
  sig_info->ber = status.dberinst;
#endif
  sig_info->per = status.dperinst;
  // ant level -> sqindicator (0~100) by NMI.
  sig_info->cninfo = sharp_bb_get_ant_value(status.sqindicator, status.nsnr);

#ifdef FEATURE_DMB_LNA_CTRL
  sharp_bb_lna_ctrl(status.sqindicator, status.rssi);
#endif
...
void sharp_bb_get_tuner_info(tIsdbtTunerInfo *tuner_info)
{
  ...
  tuner_info->ant_level = sharp_bb_get_ant_value(status.sqindicator, status.nsnr);
#ifdef FEATURE_USE_AVG_BER  
  tuner_info->ber = status.dber;
#else
