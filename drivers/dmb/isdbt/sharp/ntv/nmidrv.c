...
#ifdef FEATURE_USE_AVG_BER
		pd->sq[cix].npol++;
		//ISDBT_MSG_SHARP_BB("AVGBER %d (%d) = (%d/%d) SNR [%d]",p->dber, p->dberinst, ber.bec,ber.becw,p->nsnr);
#endif
	} else {
		p->dberinst = p->dber = 9999;
		p->nsnr = 0;
#ifdef FEATURE_USE_AVG_BER
...
void nmi_drv_video(tNmDrvMode mode, tNmDtvStream *p, int enable)
{
	...
	if (enable) {
		...
		if(tmcc_info_change > 0) 
		{  
		  dPrint(N_INFO, "TMCC change [%d]\n",tmcc_info_change); 
		  tmcc_info_change = 0;
		}
	}
}
