...
static int _cyttsp4_reading_divided(struct cyttsp4 *ts, int cmd, int mode, int offset, int elements, uint8_t *pdata)
{
    ...
    if(ts->bus_ops->tsdebug >= CY_DBG_LVL_2) {
        dev_info(ts->dev, "offset=%d elements=%d ",offset,elements);
        for(i=0;i<6;i++)
...
static int cyttsp4_chargermode(struct cyttsp4 *ts, uint8_t enable)
{
    ...
    if (ts->bus_ops->tsdebug >= CY_DBG_LVL_1)
        dev_info(ts->dev,"charger_mode=%d\n",ts->charger_mode);
#endif /* --CONFIG_TOUCHSCREEN_DEBUG */
...
{
    const int MAX_I2C_BUFFER = 256;
    u8 offset;
    u8 elements = (MAX_I2C_BUFFER/Y_SENSOR_NUM) * Y_SENSOR_NUM;
    uint8_t exec_full_panel_scan_cmd[6] = {0x0B, 0x00, 0x00, 0x00, 0x00, 0x00 };
    uint8_t read_retrieve_data[X_SENSOR_NUM*Y_SENSOR_NUM];
    ...
    for(loop_count=0;loop_count<(int)(X_SENSOR_NUM*Y_SENSOR_NUM)/MAX_I2C_BUFFER;loop_count++) {
        // read into i2c max value that does not exceed the limits of(i2c max length is 256byte)
        offset = loop_count * (int)(MAX_I2C_BUFFER/Y_SENSOR_NUM) * Y_SENSOR_NUM;
        _cyttsp4_reading_divided(ts, 0x0C, mode, offset, elements, &read_retrieve_data[offset]);
    }
    // read the rest of the x sensor.
    offset = loop_count * (int)(MAX_I2C_BUFFER/Y_SENSOR_NUM) * Y_SENSOR_NUM;
    elements = X_SENSOR_NUM*Y_SENSOR_NUM - offset;
    _cyttsp4_reading_divided(ts, 0x0C, mode, offset, elements, &read_retrieve_data[offset]);
    ...
    case CY_BALANCED_RAW:
        ...
        break;
    case CY_MUTUAL_BASELINE:
    case CY_SELF_BASELINE:
    case CY_BALANCED_BASELINE:
    case CY_MUTUAL_DIFF:
    case CY_SELF_DIFF:
    case CY_BALANCED_DIFF:
        retval = 1;
        break;
    }
    for(i=0;i<X_SENSOR_NUM;i++) {
        for(j=0;j<Y_SENSOR_NUM;j++) {
            if(retval)
                ...
                reference_data[X_SENSOR_NUM*j+i] = read_retrieve_data[Y_SENSOR_NUM*i+j];
        }
    }
...
static int cyttsp4_panel_test(struct cyttsp4 *ts) 
{
...
#else
	int loop=0;
	retval = cyttsp4_testmode(ts, CY_MUTUAL_DIFF);
	msleep(200);
	retval = cyttsp4_testmode(ts, CY_MUTUAL_DIFF);
	msleep(200);
	retval = cyttsp4_testmode(ts, CY_MUTUAL_DIFF);
    ...
    for(loop=0;loop<sizeof(reference_data)/sizeof(int);loop++) {
        if(reference_data[loop] < -50 || reference_data[loop] > 10) {
            // fail
