...
//LS2_USB tarial added audio_source for AOA 2.0 audio device (36 -> 40)
#define MAX_DEVICE_TYPE_NUM   40
...
static void pantech_usb_function_set_enabled(char *name)
{
  struct android_usb_function **functions;
	...
	//LS2_USB tarial audio_source for AOA 2.0 audio device
...
//Tarial QXDM mask control 
#ifdef FEATURE_PANTECH_QXDM_MASK_ONOFF
static ssize_t qxdm_message_mask_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int value;

	value = qxdm_mask_control;
	return sprintf(buf, "%d\n", value);
}

static ssize_t qxdm_message_mask_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	int value;

	sscanf(buf, "%d", &value);
	qxdm_mask_control = value;

	return size;
}
static DEVICE_ATTR(qxdm_message_mask, S_IRUGO | S_IWUSR, qxdm_message_mask_show, qxdm_message_mask_store);
#endif
...
//Tarial QXDM mask control
#ifdef FEATURE_PANTECH_QXDM_MASK_ONOFF
  &dev_attr_qxdm_message_mask.attr,
#endif
  NULL,
};
