...
#define PN544_DBG(fmt, args...) printk("PN544_NFC[%d]: " fmt, __LINE__, ##args)
#else 
#define PN544_DBG(fmt, args...)
#endif
...
static irqreturn_t pn544_dev_irq_handler(int irq, void *dev_id)
{
    ...
    PN544_DBG("(ven, firm, irq)=(%d, %d, %d)\n",  gpio_get_value(pn544_dev->ven_gpio), gpio_get_value(pn544_dev->firm_gpio), gpio_get_value(pn544_dev->irq_gpio));
		...
		size_t count, loff_t *offset)
{
	...
	if (ret != count) {
		PN544_DBG("i2c_master_send returned %d\n", ret);
		ret = -EIO;
	}
