...
void cbus_init_timer(u8 data, unsigned int msecs);
void uvirq_init_timer(u8 data, unsigned int msecs);
...
struct timer_list cbus_timer;
struct timer_list uvirq_timer;

boolean resen_check_init = FALSE;
boolean rsen_timer = FALSE;  // kkcho_1118
boolean cbus_timer_check = FALSE;  // kkcho_1118
boolean uvirq_timer_check = FALSE;  // kkcho_1118
...
void WriteInitialRegisterValues ( void )
{
	...
	printk("CHIP ID=%x,%x\n", I2C_ReadByte(PAGE_0_0X72, 0x02),I2C_ReadByte(PAGE_0_0X72, 0x03));
	// Power Up
...
static void cbus_cmd_send(struct work_struct * work)
{
	...
	cbus_init_timer(0,T_SRC_CMD_SEND_CHK);
	kfree(dev);
}
...
void sii9244_interrupt_schdule(void)
{
	...
	if(uvirq_timer_check==TRUE)
	{
		del_timer(&uvirq_timer);
		uvirq_timer_check=FALSE;
	}
#else
...
void sii9244_cancel_intr_work(void)
{
	cancel_delayed_work(&f_mhl_cable_check_control.work);

	if(cbus_timer_check==TRUE)
	{
		del_timer(&cbus_timer);
		cbus_timer_check=FALSE;
}
}
/*===========================================================================
// kkcho_1212
...
void hpd_change_check(void)
{
	if(mhl_status_value.mhl_status==MHL_DISCOVERY_SUCCESS)
	{
		uvirq_init_timer(0,T_SRC_UVIRQ_CHK);
	}
}
...
//        mhl_status_value.mhl_status = MHL_RSEN_LOW;
	...
	if(mhl_status_value.mhl_status==MHL_DISCOVERY_SUCCESS)
	{
		int intr1 = I2C_ReadByte(PAGE_0_0X72, 0x71);

		if(intr1&MDI_HPD)
		{
			/// Timer HPD Check.
			printk("Timer HPD Check intr1=%x\n",intr1);
...
void cbus_timer_handler(unsigned long timer_type)
{
#ifdef MHL_DEBUG
	printk(KERN_INFO "\n#### cbus_timer_handler %d ####\n", (int) timer_type);
#endif
	////CBUS interrupt check ?
	if(cbus_timer_check==TRUE)
		del_timer(&cbus_timer);
	...
	// if it failed then reset the mhl chip ?
	
}

void uvirq_timer_handler(unsigned long timer_type)
{
#ifdef MHL_DEBUG
	printk(KERN_INFO "\n#### uvirq_timer_handler %d ####\n", (int) timer_type);
#endif
	////CBUS interrupt check ?
	if(uvirq_timer_check==TRUE)
		del_timer(&uvirq_timer);
...
void cbus_init_timer(u8 data, unsigned int msecs)
{
	...
	cbus_timer_check=TRUE;
	
	init_timer(&cbus_timer);
	cbus_timer.function = cbus_timer_handler;
	cbus_timer.data = data;
	cbus_timer.expires = jiffies + (MSECS_TO_JIFFIES(msecs) + 1); // 2 * HZ;
	add_timer(&cbus_timer);
}
...
void uvirq_init_timer(u8 data, unsigned int msecs)
{
	...
	uvirq_timer_check=TRUE;
	
	init_timer(&uvirq_timer);
	uvirq_timer.function = uvirq_timer_handler;
	uvirq_timer.data = data;
	uvirq_timer.expires = jiffies + (MSECS_TO_JIFFIES(msecs) + 1); // 2 * HZ;
	add_timer(&uvirq_timer);
}
void process_mhl_discovery_start(void)
{
	u8 int4Status, intr1;
...
void set_mhl_status(int status)
{
	mhl_status_value.mhl_status = status;
}
void del_mhl_rsen_timer(void)
{
	if(rsen_timer == TRUE)
	del_timer(&simg_timer);
	rsen_timer = FALSE;
