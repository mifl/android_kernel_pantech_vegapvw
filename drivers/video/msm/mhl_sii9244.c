...
#define CONFIG_SKY_FIX_MHL_RESET_WHILE_RESTART		/// p16008 . MARUKO PLM 4844, MHL was not detected when phone restart
...
extern  cbus_cmd_node *cbus_cmd_head, *cbus_cmd_tail;
...
void is_mhl_cable(struct work_struct *work)
{
...
#if defined(T_SIRIUSLTE) && (BOARD_VER >= WS11)
	printk("%s: mhlsii9244_adc_value is %d\n",__func__, mhlsii9244_adc_value);
	adc_value = mhlsii9244_adc_value;
	
	if (!MHL_Get_Cable_State() && (adc_value > 21000	&& adc_value < 50000))///MIN=22000, MAX=50000
#else
	adc_value = (int)result.physical;
	MHL_DEV_INFO("%s: mhlsii9244_adc_value is %d\n",__func__, adc_value);
	
	if (!MHL_Get_Cable_State() && (adc_value > 0	&& adc_value < 50000))///MIN=0, MAX=50000
#endif
	{		
		atomic_set(&mhl_connect_status,0);
		do{
	...
	else if (MHL_Get_Cable_State() && (adc_value> 250000))
	{
		/* Nothing to DO */
	}
#if defined(T_SIRIUSLTE) && (BOARD_VER >= WS11)	
	else if (!MHL_Get_Cable_State() && (adc_value <= 21000))
	{
		if (is_mhl_mode())
		{
...
bool mhl_cable_connect(void)
{
	...
	r = wait_event_timeout(mhl_connect_wait_queue, get_mhl_status()==MHL_CABLE_CONNECT ,msecs_to_jiffies(600));
	...
	MHL_DEV_INFO("%s: in\n ", __func__);	
		...
		if(MHL_Get_Cable_State()){
				...
				//	mhl_power_ctrl(0);						
					...
					r = wait_event_timeout(mhl_disconnect_wait_queue, mdp4_extn_disp == 0 ,msecs_to_jiffies(3000));				
					if ( r ==0 && mdp4_extn_disp == 0)
					mhl_do_hpd_ctrl(0);
					SwitchToD3();
